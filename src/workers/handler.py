...
def change_message_visibility(receipt_handle: str, queue_url: str, visibility_timeout: int):
    """
    Smart Backoff.
    ...
    try:
        ...
        logger.info(f"Extended visibility timeout by {visibility_timeout}s due to rate limit.")
    except Exception as e:
        logger.error(f"Failed to change message visibility: {str(e)}")

def update_job_status(job_id: str, status: str, result: Dict = None, metrics: Dict = None, error_reason: str = None):
...
def process_message(record: Dict[str, Any]):
    ...
    try:
        ...
        logger.warning(f"{log_prefix} Rate limited by provider. Triggering exponential backoff of {visibility_timeout}s (Attempt {receive_count}). Details: {str(rle)}")
        
        # Extend visibility timeout so SQS waits exactly that long before letting another worker grab it.
        change_message_visibility(receipt_handle, settings.SQS_QUEUE_URL, visibility_timeout)
        # We raise the exception so Lambda knows the execution failed and leaves the message in the queue.
        raise
        
    except Exception as e:
        logger.error(f"{log_prefix} Fatal error during processing: {str(e)}", exc_info=True)
